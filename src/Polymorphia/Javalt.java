...
public class Javalt implements appliquerEffet {
	...
	protected void setArme(Armes arme) {
		this.arme=arme;
		this.degatJavalt=arme.getDegatArme();
	}
	...
	protected void removePV(int degat) {
 	...
 	protected void attaquer(Monstre monstre) {
 		monstre.setPVMonstre(monstre.getPVMonstre()-this.degatJavalt);
	...
	@Override
	public void appliquerEffetSort(Sorts sort,Monstre monstre) {
		// TODO Auto-generated method stub
		switch(sort.getNomSort()) {
		case("mort"):
			monstre.mourir();
		case("vulnerable"):
			monstre.rendreVulnerable();
		case("affaibli"):
			monstre.setAttaqueMonstre((int)monstre.getAttMonstre()/2);
		}
